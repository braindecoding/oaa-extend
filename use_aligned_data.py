...
def list_available_alignments(output_dir='./outputs'):
    ...
    for data_file in data_files:
        ...
        vangerven_file = data_file.replace('_aligned_data.npz', '_vangerven_model.pkl')
        ...
        available_files[base_name] = {
            ...
            'vangerven_model': vangerven_file if os.path.exists(vangerven_file) else None,
...
def load_alignment_session(session_name, output_dir='./outputs'):
    ...
    # Load vangerven reconstruction model
    if session_files['vangerven_model']:
        with open(session_files['vangerven_model'], 'rb') as f:
            session_data['vangerven_model'] = pickle.load(f)
        print(f"✓ Loaded vangerven model")
...
def demonstrate_downstream_processing(session_name=None, output_dir='./outputs'):
    ...
    # Example 2: Use trained vangerven model for new predictions
    if 'vangerven_model' in session_data:
        print(f"\n2. USING TRAINED VANGERVEN MODEL")
        print("-" * 30)

        vangerven_model = session_data['vangerven_model']
        ...
        try:
            predictions = vangerven_model.predict(test_fmri)
